...
class KFoldValidation {
  ...
  doValidation(){
    ...
    for(let k=0;k<this.nFold;k++) {
      ...
      let cleanDataTrainingSpam = dataTrainingSpam.map(komentar => emojiCleaner(komentar.spam))
      let cleanDataTrainingNotSpam = dataTrainingNotSpam.map(komentar => emojiCleaner(komentar.notspam))
      ...
      for(let iter=0;iter<dataTesting.length;iter++){
        ...
        if(dataTesting[iter].label === 'spam'){
          predictResult = nbcInstance.classify(emojiCleaner(dataTesting[iter].spam))
          //DO CONFUSSION MATRIX
          if (predictResult.label == dataTesting[iter].label) {
            //trueSpam
            confussionMatrixResult.trueSpam++
          } else {
            //falseSpam
            confussionMatrixResult.falseSpam++
          }
        }else if(dataTesting[iter].label === 'notspam'){
          predictResult = nbcInstance.classify(emojiCleaner(dataTesting[iter].notspam))
          //DO CONFUSSION MATRIX
          if (predictResult.label == dataTesting[iter].label) {
            // trueNotSpam
            confussionMatrixResult.trueNotspam++
          } else {
            confussionMatrixResult.falseNotspam++
          }
        } 
        if(predictResult.label == dataTesting[iter].label){
          benar+=1
