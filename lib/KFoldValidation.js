...
class KFoldValidation {
  ...
  doValidation(){
    ...
    for(let k=0;k<this.nFold;k++) {
      ...
      let cleanDataTrainingSpam = dataTrainingSpam.map(komentar => emojiCleaner(komentar.spam).normalize('NFD').replace(/[\u0300-\u036f]/g, ""))

      let cleanDataTrainingNotSpam = dataTrainingNotSpam.map(komentar => emojiCleaner(komentar.notspam).normalize('NFD').replace(/[\u0300-\u036f]/g, ""))
      ...
      for(let iter=0;iter<dataTesting.length;iter++){
        ...
        if(dataTesting[iter].label === 'spam'){
          predictResult = nbcInstance.classify(emojiCleaner(dataTesting[iter].spam).normalize('NFD').replace(/[\u0300-\u036f]/g, ""))
        }else if(dataTesting[iter].label === 'notspam'){
          predictResult = nbcInstance.classify(emojiCleaner(dataTesting[iter].notspam).normalize('NFD').replace(/[\u0300-\u036f]/g, ""))
        } 
      ...
      akurasiEachCluster = Math.round((benar/dataTesting.length)*100)
    ...
    console.log('Akurasi Model',`${akurasiRataRata/this.nFold}%`)
  }
}
